...
// Real implementation
class MemoryControllerImp(outer: MemoryController)(implicit p: Parameters) extends LazyModuleImp(outer) with HasCoreParameters {
  val io = IO(new MemoryControllerIO(outer.nRows, outer.dataBytes))
